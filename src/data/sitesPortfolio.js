// Cases de sites com página própria de apresentação.
// Para adicionar um case: criar a pasta public/sites/<slug>/ com as telas
// (tela-1.webp, tela-2.webp...) e acrescentar um bloco aqui.
export const siteProjects = [
  {
    slug: 'dra-katiuscia-souza',
    name: 'Dra. Katiuscia Souza',
    tag: 'Site Médico · Endocrinologia',
    url: 'https://drakatiusciaendocrino.com.br/',
    desc: 'Site desenvolvido no modelo de site médico para apresentar os serviços da endocrinologista Dra. Katiuscia Souza. Ela precisava de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site médico', 'Identidade visual + site', 'Agendamento de consultas'],
    slides: Array.from({ length: 6 }, (_, i) => `/sites/dra-katiuscia-souza/tela-${i + 1}.webp`),
  },
  {
    slug: 'casa-tres-arquitetura',
    name: 'Casa Três Arquitetura',
    tag: 'Site Corporativo · Arquitetura',
    url: 'https://casa3.com.br/',
    desc: 'Site desenvolvido no modelo corporativo para apresentar os serviços dos arquitetos Ricardo Nunes e Sônia Acciaris. Eles precisavam de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site corporativo', 'Identidade visual + site', 'Reposicionamento de marca'],
    slides: Array.from({ length: 9 }, (_, i) => `/sites/casa-tres-arquitetura/tela-${i + 2}.webp`),
  },
  {
    slug: 'jorge-fogaca',
    name: 'Jorge Fogaça',
    tag: 'Site Corporativo · Advocacia',
    url: 'https://fogacasantos.com.br/',
    desc: 'Site desenvolvido no modelo corporativo para apresentar os serviços do advogado previdenciarista Jorge Fogaça. Ele precisava de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site corporativo', 'Reposicionamento de marca'],
    slides: Array.from({ length: 5 }, (_, i) => `/sites/jorge-fogaca/tela-${i + 2}.webp`),
  },
  {
    slug: 'dra-karina-morgarbel',
    name: 'Dra. Karina Morgarbel',
    tag: 'Site Médico · Dermatologia',
    url: null,
    desc: 'Site desenvolvido no modelo de site médico para apresentar os serviços da dermatologista Dra. Karina Morgarbel. Ela precisava de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site médico', 'Identidade visual + site'],
    slides: Array.from({ length: 6 }, (_, i) => `/sites/dra-karina-morgarbel/tela-${i + 2}.webp`),
  },
  {
    slug: 'claudia-kaissar',
    name: 'Claudia Kaissar',
    tag: 'Site Corporativo · Arquitetura de Luxo',
    url: null,
    desc: 'Site corporativo para a arquiteta Claudia Kaissar, profissional premiada e especialista em ambientes de luxo. Cada detalhe foi pensado para valorizar as imagens dos projetos e reforçar sua autoridade no mercado. Como seu público está em diferentes países, o site foi desenvolvido em inglês e português, garantindo alcance global.',
    chips: ['Site corporativo', 'Bilíngue (PT/EN)', 'Alcance internacional'],
    slides: [
      '/sites/claudia-kaissar/claudia-tela-2.webp',
      '/sites/claudia-kaissar/tela-2-2.webp',
      '/sites/claudia-kaissar/tela-2-3.webp',
      '/sites/claudia-kaissar/tela-4-2.webp',
      '/sites/claudia-kaissar/tela-5-2.webp',
      '/sites/claudia-kaissar/tela-6-2.webp',
      '/sites/claudia-kaissar/tela-7.webp',
    ],
  },
  {
    slug: 'dr-gabriel-braga',
    name: 'Dr. Gabriel Braga',
    tag: 'Site Médico · Cirurgia',
    url: null,
    desc: 'Site desenvolvido no modelo de site médico para apresentar os serviços do cirurgião Dr. Gabriel Braga. Ele precisava de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site médico'],
    slides: Array.from({ length: 7 }, (_, i) => `/sites/dr-gabriel-braga/tela-${i + 2}.webp`),
  },
  {
    slug: 'raquel-barros',
    name: 'Raquel Barros',
    tag: 'Landing Page de Captura · Micropigmentação',
    url: null,
    desc: 'Landing page de captura para o evento de lançamento da especialista em micropigmentação Raquel Barros, referência na área que já formou centenas de alunos no Brasil e no mundo. Reformulamos suas páginas com um visual mais profissional e moderno, com design estratégico que preserva a imagem da especialista e organiza os elementos para aumentar as chances de conversão.',
    chips: ['Landing page de captura', 'Lançamento de evento', 'Foco em conversão'],
    slides: [
      '/sites/raquel-barros/raquel-tela-2.webp',
      '/sites/raquel-barros/raquel-tela-3.webp',
      '/sites/raquel-barros/raquel-tela-4.webp',
      '/sites/raquel-barros/raquel-tela-5.webp',
    ],
  },
  {
    slug: 'pett-capellato',
    name: 'Pett Capellato',
    tag: 'Hub Corporativo · Arquitetura e Design',
    url: 'https://pettcapellato.com.br/',
    desc: 'Hub que conecta arquitetos e designers de interiores a grandes fabricantes de mobiliário corporativo, atendendo projetos para marcas como Globo, Mercado Livre, Vale e Michelin. Construímos uma presença digital à altura dessas contas, evoluída mês a mês desde 2022.',
    chips: ['Cliente desde 2022', 'Acompanhamento mensal', 'Cases: Globo, Mercado Livre, Vale, Michelin'],
    slides: [
      '/sites/pett-capellato/tela-2.jpg',
      '/sites/pett-capellato/tela-3.jpg',
      '/sites/pett-capellato/tela-4.jpg',
      '/sites/pett-capellato/tela-5.jpg',
      '/sites/pett-capellato/tela-6.jpg',
      '/sites/pett-capellato/tela-7.webp',
      '/sites/pett-capellato/tela-8.webp',
      '/sites/pett-capellato/tela-9.jpg',
    ],
  },
  {
    slug: 'vicente-hennemann',
    name: 'Dr. Vicente Hennemann',
    tag: 'Site Médico · Fisioterapia',
    url: null,
    desc: 'Fisioterapeuta especialista em dor em Porto Alegre, professor sênior do Método McKenzie, com mais de 20 anos de atendimento e 5.000 pacientes atendidos. Site desenvolvido no modelo de site médico para apresentar seus serviços com o profissionalismo e a credibilidade que seu trabalho exige.',
    chips: ['Site médico', 'Método McKenzie', '+5.000 pacientes atendidos'],
    slides: Array.from({ length: 7 }, (_, i) => `/sites/vicente-hennemann/tela-${i + 1}.jpg`),
  },
  {
    slug: 'mateus-dias',
    name: 'Mateus Dias',
    tag: 'Landing Page de Vendas · Marketing e Funis',
    url: null,
    desc: 'Um dos maiores especialistas em marketing e referência em funis de vendas no Brasil. Landing page de vendas estruturada estrategicamente, com conteúdo organizado e imagens que destacam a autoridade do especialista, aumentando as chances de conversão.',
    chips: ['Landing page de vendas', 'Funil de vendas', 'Foco em conversão'],
    slides: [
      '/sites/mateus-dias/tela-2.webp',
      '/sites/mateus-dias/tela-3-2.webp',
      '/sites/mateus-dias/tela-4-2.webp',
      '/sites/mateus-dias/tela-5-2.webp',
      '/sites/mateus-dias/tela-6.webp',
      '/sites/mateus-dias/tela-7.webp',
    ],
  },
  {
    slug: 'daiany-oliveira-psicologa',
    name: 'Daiany Oliveira',
    tag: 'Site Médico · Psicologia',
    url: null,
    desc: 'Site desenvolvido no modelo de site médico para apresentar os serviços da psicóloga Daiany Oliveira. Ela precisava de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site médico'],
    slides: Array.from({ length: 8 }, (_, i) => `/sites/daiany-oliveira-psicologa/daiany-oliveira-tela-${i + 1}.webp`),
  },
  {
    slug: 'gomes-e-zarur-advogados',
    name: 'Gomes e Zarur Advogados',
    tag: 'Site Corporativo · Advocacia',
    url: null,
    desc: 'Site desenvolvido no modelo corporativo para apresentar os serviços do escritório Gomes e Zarur Advogados. Eles precisavam de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site corporativo'],
    slides: Array.from({ length: 7 }, (_, i) => `/sites/gomes-e-zarur-advogados/gomes-e-zarur-advogados-tela-${i + 1}.webp`),
  },
  {
    slug: 'brio-clean',
    name: 'Brio Clean',
    tag: 'Site Institucional · Higienização de Estofados',
    url: null,
    desc: 'Site desenvolvido para apresentar os serviços de higienização de estofados da Brio Clean, em Viseu, Portugal. A empresa precisava de um site à altura do seu trabalho, que transmitisse confiança e profissionalismo.',
    chips: ['Site institucional'],
    slides: [
      '/sites/brio-clean/brion-clean-tela-1.webp',
      '/sites/brio-clean/brion-clean-tela-2.webp',
      '/sites/brio-clean/brion-clean-tela-3.webp',
      '/sites/brio-clean/brion-clean-tela-4.webp',
      '/sites/brio-clean/brion-clean-tela-5.webp',
      '/sites/brio-clean/brion-clean-tela-6.webp',
    ],
  },
  {
    slug: 'edson-guilherme-psiquiatra',
    name: 'Edson Guilherme',
    tag: 'Site Médico · Psiquiatria',
    url: null,
    desc: 'Site desenvolvido no modelo de site médico para apresentar os serviços do psiquiatra Edson Guilherme. Ele precisava de um site à altura do seu trabalho, que transmitisse acolhimento e credibilidade.',
    chips: ['Site médico'],
    slides: Array.from({ length: 7 }, (_, i) => `/sites/edson-guilherme-psiquiatra/edson-psiquiatra-tela-${i + 1}.webp`),
  },
  {
    slug: 'neurolandia',
    name: 'Neurolândia',
    tag: 'Site para Psicólogos · Neuropsicologia',
    url: 'https://neurolandia.com.br/avaliacao-neuropsicologica/',
    desc: 'Consultório especializado em avaliação neuropsicológica. Desenhamos o site para conduzir o público certo até a triagem, com linguagem acessível para famílias e estrutura clara de encaminhamento.',
    chips: ['Estratégia de público'],
    slides: Array.from({ length: 10 }, (_, i) => `/sites/neurolandia/neurolandia-tela-${i + 1}.webp`),
  },
  {
    slug: 'planexec',
    name: 'Sandro Britto · PlanExec',
    tag: 'Site Corporativo · Consultoria Empresarial',
    url: null,
    desc: 'Site desenvolvido no modelo corporativo para apresentar a consultoria empresarial com cogestão de Sandro Britto, da PlanExec. Ele precisava de um site à altura da experiência que entrega aos clientes, que transmitisse profissionalismo e credibilidade.',
    chips: ['Site corporativo', 'Consultoria empresarial'],
    slides: [
      '/sites/planexec/planexec-tela-1.webp',
      '/sites/planexec/planexec-tela-2.webp',
      '/sites/planexec/planexec-tela-3.webp',
      '/sites/planexec/planexec-tela-4.webp',
      '/sites/planexec/planexec-tela-5.webp',
    ],
  },
]
